...
class RubikCube:
    ...
    def get_state_tuple(self):
        ...
        """
        # Lưu ý quan trọng: Chúng ta đã sửa lỗi đảo ngược phép quay trong hàm rotate_face(),
        # giờ đây trạng thái được lưu trữ chính xác, không cần đảo ngược nữa
        corrected_state = self.state.copy()
        
        # Trả về các tuple biểu diễn trạng thái đã hiệu chỉnh
        return (
            corrected_state.cp,  # Corner permutation (hoán vị góc)
            corrected_state.co,  # Corner orientation (định hướng góc)
            corrected_state.ep,  # Edge permutation (hoán vị cạnh)
            corrected_state.eo   # Edge orientation (định hướng cạnh)
        ) 