...
from RubikState.rubik_solver_2x2 import (
    a_star_search_2x2, greedy_best_first_search_2x2, ida_star_search_2x2, 
    hill_climbing_max_search_2x2, hill_climbing_random_search_2x2, 
    a_star_pdb_2x2, load_pattern_database
)
...
def run_algorithms(scrambled_states, depths, time_limit=30):
    ...
    # Try to load pattern database
    pdb = load_pattern_database()
    
    algorithms = {
        "A*": a_star_search_2x2,
        "Greedy Best-First": greedy_best_first_search_2x2,
        "IDA*": ida_star_search_2x2,
        "Hill Climbing (Max)": hill_climbing_max_search_2x2,
        "Hill Climbing (Random)": hill_climbing_random_search_2x2
    }
    
    # Add PDB A* if pattern database was loaded
    if pdb:
        algorithms["A* with PDB"] = lambda state, time_limit=30, return_stats=False: a_star_pdb_2x2(
            state, time_limit=time_limit, pdb=pdb, return_stats=return_stats)
...
def create_visualizations(results, depths):
    ...
    plt.title('Success Rate by Scramble Depth (2x2)', fontsize=16)
    ...
    for i, (alg, data) in enumerate(results.items()):
        plt.plot(depths, data['solution_length'], marker='o', linewidth=2, 
                 label=alg, color=colors[i])
    plt.title('Average Solution Length by Scramble Depth (2x2)', fontsize=16)
    plt.xlabel('Scramble Depth', fontsize=14)
    ...
    for i, (alg, data) in enumerate(results.items()):
        plt.plot(depths, data['time_taken'], marker='o', linewidth=2, 
                 label=alg, color=colors[i])
    plt.title('Average Solution Time by Scramble Depth (2x2)', fontsize=16)
    plt.xlabel('Scramble Depth', fontsize=14)
    ...
    for i, (alg, data) in enumerate(results.items()):
        plt.plot(depths, data['nodes_explored'], marker='o', linewidth=2, 
                 label=alg, color=colors[i])
    plt.title('Average Nodes Explored by Scramble Depth (2x2)', fontsize=16)
    plt.xlabel('Scramble Depth', fontsize=14)
    ...
    # Save the figure
    plt.savefig('visualizations/2x2_informed_search.png', dpi=300)
    print(f"Visualization saved to 'visualizations/2x2_informed_search.png'")
    
    return plt

if __name__ == "__main__":
    print("Rubik's Cube 2x2 - Informed Search Algorithms Visualization")
    print("==========================================================")
    
    # Define depths to test - 2x2 cube can handle deeper scrambles
    depths = [1, 2, 3, 4, 5, 6]
