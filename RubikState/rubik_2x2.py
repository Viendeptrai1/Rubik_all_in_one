...
MOVES_2x2 = {
    ...
    'U': {
        'cp': (3, 0, 1, 2, 4, 5, 6, 7), # Góc: 0->3, 1->0, 2->1, 3->2
        'co': (0, 0, 0, 0, 0, 0, 0, 0), # Hướng góc không đổi
    },
    # === Phép xoay mặt R (Right - phải) 90 độ CW ===
    'R': {
        'cp': (4, 1, 2, 0, 7, 5, 6, 3), # Góc: 0->4, 3->0, 7->3, 4->7
        'co': (1, 0, 0, 2, 2, 0, 0, 1), # Hướng: URF(+2), URB(+1), DRB(+2), DRF(+1)
    },
    # === Phép xoay mặt F (Front - trước) 90 độ CW ===
    'F': {
        'cp': (1, 5, 2, 3, 0, 4, 6, 7), # Góc: 0->1, 1->5, 5->4, 4->0
        'co': (2, 1, 0, 0, 1, 2, 0, 0), # Hướng: URF(+1), ULF(+2), DLF(+1), DRF(+2)
    },
    # === Phép xoay mặt D (Down - dưới) 90 độ CW ===
    'D': {
        'cp': (0, 1, 2, 3, 5, 6, 7, 4), # Góc: 4->5, 5->6, 6->7, 7->4
        'co': (0, 0, 0, 0, 0, 0, 0, 0), # Hướng góc không đổi
    },
    # === Phép xoay mặt L (Left - trái) 90 độ CW ===
    'L': {
        'cp': (0, 2, 6, 3, 4, 1, 5, 7), # Sửa thành: 1->5, 2->1, 5->6, 6->2
        'co': (0, 2, 1, 0, 0, 1, 2, 0), # Hướng: ULF(+1), ULB(+2), DLB(+1), DLF(+2)
    },
    # === Phép xoay mặt B (Back - sau) 90 độ CW ===
    "B": {
        'cp': (0, 1, 3, 7, 4, 5, 2, 6), # Góc: 2->3, 3->7, 7->6, 6->2
        'co': (0, 0, 2, 1, 0, 0, 1, 2), # Cập nhật theo quy ước của 3x3
    },
    # === Phép xoay ngược chiều (CCW - prime) - ĐÃ SỬA CO ===
    "U'": {
    ...
    "R'": {
        ...
        'co': (1, 0, 0, 2, 2, 0, 0, 1), # Hướng ngược R (Đã sửa)
    },
    "F'": {
        'cp': (4, 0, 2, 3, 5, 1, 6, 7), # Nghịch đảo cp của F
        'co': (2, 1, 0, 0, 1, 2, 0, 0), # Hướng ngược F (Đã sửa)
    },
    "D'": {
    ...
    "L'": {
        'cp': (0, 5, 1, 3, 4, 6, 2, 7), # Nghịch đảo cp của L (cập nhật theo cp L mới)
        'co': (0, 2, 1, 0, 0, 1, 2, 0), # Hướng ngược L (Đã sửa)
    },
    "B'": {
